...
이전 Python 고정 일정 계약은 `legacy.html`에 대한 보존 검사입니다. `tests/browser_smoke.cjs` 역시 이전 화면용이며, 2026-09-11 확인 시점에는 전체 통과합니다(이전에 적어둔 화요일 탭 알려진 이슈는 더 이상 재현되지 않아 문구를 정리했습니다). 현재 편집기 성공 근거와 섞지 않습니다.

### 시각은 Europe/Paris 실제 순간입니다

Notion 날짜 속성이 워크스페이스의 Asia/Seoul 오프셋으로 저장되는 바람에, 파리 현지 시각으로 적은 숫자가 **7시간 이른 순간**으로 들어가 있었습니다(예: 로댕 16:40 → `07:40Z`). 2026-09-11에 Notion 원본과 스냅샷을 모두 실제 파리 순간으로 교정했습니다. `tests/test_editor_seed.py`가 각 일정의 파리 벽시계 시각을 고정해 재발을 막습니다. 새 일정을 넣을 때도 **파리 현지 시각**을 기준으로 입력하세요.
